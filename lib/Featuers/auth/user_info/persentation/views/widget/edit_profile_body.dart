...
class EditUserInfoViewState extends State<EditUserInfoView> {
  ...
  late TextEditingController bioController;
  XFile? profileImage;
  String? profilePicUrl; // Local variable for profile pic URL
  bool isLoading = false;
  ...
  void initState() {
    ...
    bioController = TextEditingController(text: widget.userInfo.bio);
    profilePicUrl =
        widget.userInfo.profilePic; // Initialize with existing profilePic
  }
  ...
  Future<void> _pickImage() async {
    ...
    await ShowDialogClass.imagePickerDialog(
      ...
      removeFCT: () {
        setState(() {
          profileImage = null; // Clear selected profile image
          profilePicUrl = null; // Clear profilePic URL
        });
        succesTopSnackBar(
          context,
          'Profile image removed successfully',
  ...
  Future<void> _updateUserInfo() async {
    ...
    try {
      // If the user selects a new image, upload it
      if (profileImage != null) {
        // Upload the selected image to Firebase
        profilePicUrl = await _uploadImageToFirebase(profileImage!.path);
      }

      // If the user explicitly removed the image (profileImage == null and profilePicUrl is also null)
      if (profileImage == null &&
          profilePicUrl == null &&
          widget.userInfo.profilePic != null) {
        // Remove the image from Firebase if it was previously stored
        await _deleteImageFromFirebase(widget.userInfo.profilePic!);
      }
      ...
      final updatedUserInfo = {
        'name': nameController.text,
        'bio': bioController.text,
        'profilePic': profilePicUrl, // Will keep existing image if not removed
      };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: CustomProgrssHud(
        isLoading: isLoading, // Display loading indicator if isLoading is true
        child: Padding(
          padding: const EdgeInsets.all(16.0),
          child: SingleChildScrollView(
            child: Column(
